...
## 本地存储格式更新（2026-07-22）
当前每个面板使用**单一 key** `panel:<名称>` 存储 `{width,height,type,createTime,cells:[...]}`（含全部宫格内容），
消除了旧版「每格一个独立 key」的 key 数量爆炸问题（例如月历 42 格从 42 个 key 降为 1 个）。
历史版本使用的 `<名称>-data` / `<名称>-text-<i>` 散落存储，会在首次启动时由 `JSON_STORAGE.migratePanels()` 自动合并迁移，无需手动操作。
